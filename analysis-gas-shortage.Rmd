...
baseLinePlot +
  geom_area(
    ...
    aes(x = plot_date, y = full_avg), 
    fill = "white") +
  geom_line(
    data = df_gas_storage_eu,
    aes(x = plot_date, y = full, group = year, colour = year >= 2021, size = year >= 2021)) +
  geom_text(
    data = subset(df_gas_storage_eu, gasDayStartedOn == max(gasDayStartedOn, na.rm = T)),
    aes(x = plot_date, y = full, label = format(gasDayStartedOn, "%d/%m/%y")), 
    size = 3, colour = "black", hjust = 1, nudge_y = -5) +
  geom_text(
    data = subset(df_gas_storage_eu, gasDayStartedOn == max(gasDayStartedOn, na.rm = T)),
    aes(x = plot_date, y = full, label = paste0(round(full), "%")), 
    size = 6, colour = "black", hjust = 1, nudge_y = -10, fontface = "bold")
...
baseLinePlot +
  ...
  geom_text(
    ...
    aes(x = plot_date, y = full, label = paste0(round(full), "%")), 
    size = 4, colour = "black", hjust = 1, nudge_y = -10, fontface = "bold") +
...
countries_middle <- latest_data %>% 
  filter(diff > -15  & diff < 5)
...
baseLinePlot +
  ...
  geom_text(
    ...
    aes(x = plot_date, y = full, label = paste0(round(full), "%")), 
    size = 4, colour = "black", hjust = 1, nudge_y = -10, fontface = "bold") +
  facet_wrap(~country_name_nl, ncol = 2, scales = "free_x")
...
basePlot +
  ggalt::geom_dumbbell(
    data = df_gas_diff,
    aes(x=full, y = reorder(country_name_nl, full), xend=full_avg),
    size=0.5, size_x = 3, size_xend = 3, colour = "black", colour_x = "black", colour_xend = "white", dot_guide=FALSE) +
...
gas_price <- fromJSON("https://www.theice.com/marketdata/DelayedMarkets.shtml?getHistoricalChartDataAsJson=&marketId=5325990&historicalSpan=3") %>%
  as.data.frame() %>% 
...
gas_price %>% 
  ...
  geom_vline(xintercept = seq.Date(ymd("2020-01-01"),ymd("2022-01-01"), "year"), size = 0.2) +
  geom_area(alpha = 0.4) +
  geom_line() +
